...
export default class PlaceOrderUseCase implements UseCaseInterface {
    ...
    constructor(clientFacade: ClientAdmFacadeInterface, productFacade: ProductAdmFacadeInterface, catalogFacade: StoreCatalogFacadeInterface, invoiceFacade: InvoiceFacadeInterface, paymentFacade: PaymentFacadeInterface, repository: CheckoutGateway) { 
    ...
    async execute(input: PlaceOrderInputDto): Promise<PlaceOrderOutputDto> {
        ...
        const invoice = payment.status == 'approved' 
            ? await this._invoiceFacade.generate({
                ...
                state: client.state,
                zipCode: client.zipCode,
                items: products.map(p => ({ id: p.id.id, name: p.name, price: p.salesPrice })),
              })
            : null
